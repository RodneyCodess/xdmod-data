...
# this gives the xdmod-11-0 container a little extra time to start up before we try it with requests, otherwise a race condition can cause a connection refused error
...
# the min Python version needs the system CA bundle for pip to reach pypi
